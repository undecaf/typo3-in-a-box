...
| `--show`<br>`-s` | `run`<br>`stop`<br>`composer`<br>`shell`<br>`env` | If this option is present then it shows generated Docker/Podman commands on the console.<br>_Warning:_ your database credentials will be visible on the console if this option is set for `t3 run`.<br>Default: `$T3_SHOW`, or not set. |
